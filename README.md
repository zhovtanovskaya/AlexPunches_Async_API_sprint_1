...
Вебсокет-сервис для "Кино вместе" в директории `ws_together`
Запустить вебсокет-сервис
```bash
python ws_together/src/main.py
```

Фронтенд в директории `frontend_together`
запустить сервер для фронтенда
```bash
...
Для демонстрации:  
авторизация отключена (закоментирована)
рандомно будет выпадать роль вердущего

В планах добавить загрузку всей истории чата для опоздавших и для переподключившихся,
сейчас они получают только актуальное состояние плеера (плей, пауза, таймкод, скорость....).
